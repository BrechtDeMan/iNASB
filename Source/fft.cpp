...
void FFT::InitCosSinTable() {
    ...
    i = 0;
    while (dftsize <= NFREQ) {
        theta = (float)(-2.0f*M_PI/(float)dftsize);
...
void FFT::time_to_frequency_domain(float* in_wavedata, float* out_spectraldata) {
    ...
    // Each time the frequency floats, that sounds like going up an octave.
...
TSpectrum::TSpectrum(TParams& params)
    : Window(params.Window)
    , FreqCount(params.FreqCount)
{
    FFTrans.Init(Window, FreqCount, 0, -1);
}

TSpectrum::~TSpectrum() {
}

void TSpectrum::AmplitudeSpectrum(std::vector<float>& origSignal, std::vector<float>& freq) {
    /*size_t sampleCount = origSignal.size()/sizeof(float);
    Init(freq, sampleCount);

    freq.resize(FreqCount);*/
    FFTrans.time_to_frequency_domain(&origSignal.front(), &freq.front());
}

void TSpectrum::Init(std::vector<float>& freq, size_t sampleCount) {
    /*if (sampleCount >= Window) {
        size_t chunkCount = (sampleCount - Window) / Shift + 1;
        freq.resize(chunkCount);
    }*/
}
